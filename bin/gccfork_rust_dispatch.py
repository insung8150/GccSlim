...
세 가지 subcommand 가 Rust 바이너리 (`gccslim-slim`,
`gccslim-claude-patch`) 가 있으면 위임, 없거나 비활성이면
Python fallback 으로 떨어짐.

스위치 우선순위:
    1. 환경변수 `GCCSLIM_DISABLE_RUST=1` — Python 강제
    2. 환경변수 `GCCSLIM_FORCE_RUST=1` — Rust 가 없거나 실패해도 fallback 안 함 (디버그)
...
def _first_existing_binary(env_names: tuple[str, ...], names: tuple[str, ...]) -> Path:
...
RUST_SLIM_BIN = _first_existing_binary(
    ("GCCSLIM_SLIM_BIN", "GCCFORK_SLIM_BIN"),
    ("gccslim-slim", "gccfork-slim"),
)
RUST_PATCH_BIN = _first_existing_binary(
    ("GCCSLIM_PATCH_BIN", "GCCFORK_PATCH_BIN"),
    ("gccslim-claude-patch", "gccfork-claude-patch"),
)


def rust_enabled() -> bool:
    """현재 Rust 위임이 활성화되어 있는가."""
    if os.environ.get("GCCSLIM_DISABLE_RUST") == "1" or os.environ.get("GCCFORK_DISABLE_RUST") == "1":
        return False
    if os.environ.get("GCCSLIM_FORCE_RUST") == "1" or os.environ.get("GCCFORK_FORCE_RUST") == "1":
        return True
    return RUST_DEFAULT
...
def _force_rust() -> bool:
    return os.environ.get("GCCSLIM_FORCE_RUST") == "1" or os.environ.get("GCCFORK_FORCE_RUST") == "1"
...
def try_rust_slim_inplace(args) -> Optional[int]:
    ...
    if not _rust_bin_ok(RUST_SLIM_BIN):
        if _force_rust():
            print("❌ GCCSLIM_FORCE_RUST=1 인데 gccslim-slim 바이너리 없음", file=sys.stderr)
            return 5
        return None
    ...
    if af:
        ...
        # GCCSLIM_VISIBLE_CAP 또는 args.visible_cap → 안 보이는 영역 압축본화
        ...
        if vc is None:
            import os as _os
            v_env = (_os.environ.get("GCCSLIM_VISIBLE_CAP") or _os.environ.get("GCCFORK_VISIBLE_CAP") or "").strip()
            if v_env.isdigit():
                vc = int(v_env)
            else:
...
def try_rust_slim_and_reload(args) -> Optional[int]:
    """`gccslim slim-and-reload --self|--sid <sid>` 의 Rust 위임 시도."""
    if not rust_enabled():
        return None
    if not _rust_bin_ok(RUST_SLIM_BIN):
        if _force_rust():
...
def try_rust_patch_claude(args) -> Optional[int]:
    """`gccslim patch-claude` 의 Rust 위임 시도."""
    if not rust_enabled():
        return None
    if not _rust_bin_ok(RUST_PATCH_BIN):
        if _force_rust():
            print("❌ GCCSLIM_FORCE_RUST=1 인데 gccslim-claude-patch 바이너리 없음",
                  file=sys.stderr)
            return 5
        return None
...
def status_summary() -> str:
    ...
    flag = (
        "DISABLE" if os.environ.get("GCCSLIM_DISABLE_RUST") == "1" or os.environ.get("GCCFORK_DISABLE_RUST") == "1"
        else "FORCE" if os.environ.get("GCCSLIM_FORCE_RUST") == "1" or os.environ.get("GCCFORK_FORCE_RUST") == "1"
        else "default"
    )
    return (
        f"Rust 위임: {'ON' if enabled else 'OFF'}  (스위치: {flag}, RUST_DEFAULT={RUST_DEFAULT})\n"
        f"  gccslim-slim         : {'✅' if slim_ok else '❌'} {RUST_SLIM_BIN}\n"
        f"  gccslim-claude-patch : {'✅' if patch_ok else '❌'} {RUST_PATCH_BIN}"
    )

...
> 📸 _아래는 플레이스홀더입니다 — 실제 TUI 캡처로 교체하세요. `gccslim` 실행 장면을 담은 짧은 GIF가 저장소 페이지에서 가장 효과가 큽니다._

| 메인 세션 화면 | 슬림 처리 동작 |
|---|---|
| ![GccSlim 메인 TUI](assets/screenshot-main.png) | ![GccSlim 슬림 워크플로우](assets/screenshot-slim.png) |

<!--
할 일 (스크린샷):
  1. `gccslim` TUI를 캡처해서 assets/screenshot-main.png 로 저장
  2. 슬림/재시작 실행 장면을 캡처해서 assets/screenshot-slim.png 로 저장
  3. (권장) 짧은 GIF를 녹화해 이 섹션 상단에 삽입
  완료 후 위 플레이스홀더 안내 문구는 삭제하세요.
-->
